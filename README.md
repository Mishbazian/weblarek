...
- Базовый класс для отображения форм. Расширяет Component. Дженерик, принимает T - тип дополнительных данных формы для метода render().\Эмитирует браузерные события change и submit с данными формы через полученный в конструкторе интерфейс брокера сообщений и выводит сообщения об ошибках.

Конструктор:

- `constructor(events: IEvents, container: HTMLElement)` - принимает интерфейс брокера сообщений и ссылку на DOM элемент, содержащий форму.
...
`order:complete`
- закрытие модального окна кнопко или кликом вне области
`modal:close`
