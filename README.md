...
Наследует `CardView<TCardExt & T>` и добавляет работу с изображением и категорией товара.
Принимает в собственную переменную `T` тип данных, расширяющий данные принимаемые методом `remder()` родительского компонента. 
...
– карточка товара в каталоге (наследует `CardExtView<never>`).
...
– карточка товара в режиме предпросмотра (наследует `CardExtView<TCardPreview>`).
