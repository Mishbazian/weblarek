...
- `setSelectedProduct(product: IProduct): void` - записывает объект данных товара полученный в аргументе в поле SelectedProduct. Эмитирует событие `model:catalogue:select` с данными выбранного товара через интерфеййс брокера сообщений.
...
- `private notify(): void` - эмитирует событие `model:cart:update` через брокер сообщений
...
- `private notify(): void` - эмитирует событие `model:buyer:update` через брокер сообщений
...
.Абстрактный класс для карточки товара с дополнительными полями (категория, изображение) Расширяет CardView.
 T - тип дополнительных данных карточки, объединяемый с TCardExt.
...
- `constructor(events: IEvents, container: HTMLElement)` - принимает интерфейс брокера сообщений и ссылку на DOM элемент модального окна. Устанавливает обработчик клика передающий событие `modal:close` через брокер.
...
- `constructor(events: IEvents, container: HTMLElement)` - принимает принимает интерфейс брокера сообщений и ссылку на DOM элемент, содержащий разметку корзины. 
...
- `constructor(events: IEvents, container: HTMLElement)` - принимает интерфейс брокера событий и ссылку на DOM элемент с разметкой окна. Устанавливает обработчик клика передающий событие `order:complete` через брокер.
...
- изменение данных покупателя\
  `model:buyer:update` событие, генерируемое моделью Buyer при любом изменении данных. Вместе с событем передается объект с обновленными данными покупателя.\
