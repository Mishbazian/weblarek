...
- `private notify(): void` - эмитирует событие `model:cart:update` через брокер сообщений. Высылает данные формата `TCartData` (массив товаров, стоимость и количество товаров в корзине).
...
- `private notify(): void` - эмитирует событие `model:buyer:update` через брокер сообщений. Высылает данные формата `TBuyerData` (сохраненные данные и результаты валидации).
